...
//		System.out.println("row " + row + " col " + col);
	...
	/***
	 * Initialize the Herbivores, Carnivores and Plants in the world.
	...
	private void initializePopulation() {
		...
		// Create Herbivores
		for (int numHerb = 0; numHerb < RunWorld.getHerbVal(); numHerb++) {
		...
		// Create Plants
		for (int numPlant = 0; numPlant < RunWorld.getPlantVal(); numPlant++) {
		...
		// ADDED CARNIVORS
		for (int numCarn = 0; numCarn < RunWorld.getCarnoVal(); numCarn++) {
			Carnivors h = new Carnivors(rng.nextDouble()*heightInPixels, rng.nextDouble()*widthInPixels);
			//			System.out.println("startX: " +  h.getLocationX() + " startY: " + h.getLocationY());
	...
	public void hunt(Lifeform h) {
		...
		while (i < getPopulationSize()) {
			...
			if (h != otherLife && !otherLife.getToBeRemoved() && h.canEat(otherLife) && h.isNear(otherLife, 50.0)) {
				h.eat(otherLife.getFoodValue());
//				System.out.println("Eating " + i);
				otherLife.setToBeRemoved(true);
				break;
			}
			i++;
		}
	 ...
	 * @return
	 */
	public int numCarnivors() {
		int numCarnivors = 0;
		for (int i = 0; i < getPopulationSize(); i++) {
			if (population.get(i) instanceof Carnivors)
				numCarnivors++;
		}
		return numCarnivors;
	}
	...
	public void simulateAll() {
		...
		while ( index < population.size() ) {
			Lifeform l = population.get(index);
		...
		index = 0;
		while (index < population.size()) {
			if (population.get(index).getToBeRemoved()) {
				population.remove(index);
			}
			else
				index++;
		...
		if (day%50 == 0)
			...
			System.out.println("" + "day:\t" + day + "\t" + "Carnivors: " + "\t" + numCarnivors() + "\t" + "Herbivores: " + "\t" + numHerbivores() + "\t" + "Plants: " + "\t" + numPlants());
	}
	public int getDay(){
		return day;
	}
